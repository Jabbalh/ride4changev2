...
- Polices : Bebas Neue (titres), Barlow Condensed (menus, labels) et Barlow (texte), servies localement via `@fontsource/*` et importées dans `src/main.ts` en sous-ensemble latin seul (`@fontsource/<police>/latin-<poids>.css`, qui couvre le français). Un nouveau `font-weight` doit y être importé sous cette forme. Ne pas réintroduire Google Fonts ni d'autres ressources tierces (seule exception : l'API Supabase) : le choix a été fait pour le RGPD.
...
- Images de `public/` en **WebP** (convertir toute nouvelle photo, redimensionnée à sa taille d'affichage ×2), avec `width`, `height`, `loading="lazy"` et `decoding="async"` sur les `<img>` (la règle globale `img { height: auto }` évite toute déformation). Seules exceptions en JPEG : `og-image.jpg` et `logo.jpg` (aperçus de partage et JSON-LD). `logo.svg` a été optimisé avec SVGO.
- `public/_headers` : en-têtes de sécurité (CSP, nosniff, HSTS, frame-ancestors…) et cache (`/assets/*` un an, images une semaine). La CSP n'autorise que le domaine Supabase du projet : à mettre à jour si le projet change, et à compléter pour toute nouvelle ressource externe (sinon elle est bloquée). Les réponses du worker (`/api/*`, `/sitemap.xml`) reçoivent leurs en-têtes dans `worker/index.ts`.
- Découpage du JS : seul l'accueil est importé en statique dans le router, les autres vues sont chargées à la demande.
- Accessibilité (à conserver) : contour `:focus-visible` global et bloc `prefers-reduced-motion` dans `main.css` (classe `.sr-only` disponible) ; lien d'évitement « Aller au contenu » et `<main id="contenu">` dans `App.vue` ; chaque champ de formulaire a un `<label for>` ; éléments cliquables en `<button>` (jamais `<div @click>`) ; popins en `<dialog>` natif (`showModal()`, Échap et focus gérés par le navigateur) ; images avec `alt` descriptif, ou `alt=""` si décoratives ; liens externes avec `aria-label="… (nouvel onglet)"` et `rel="noopener"`.
- Mentions légales et politique de confidentialité : `src/views/LegalView.vue` (`/mentions-legales`), liée dans le pied de page et sous le formulaire de contact (mention RGPD obligatoire). Les informations propres à l'association sont des constantes en haut du fichier ; une valeur vide s'affiche « à compléter ». À mettre à jour si un traitement de données change (envoi des messages, outil de mesure d'audience, cookies…).
- Galerie et Compétition : même composant `src/components/PhotoGallery.vue` (en-tête, filtres, grille, popin) ; les vues ne contiennent que leurs données.

### Supabase (contenu)
- Le navigateur interroge Supabase directement, sans passer par le worker. Lectures **publiques** : simple `fetch` REST (`src/lib/publicApi.ts`, `restSelect`/`restSelectOne`, syntaxe PostgREST) dans `useEvents.ts` et `useMilestones.ts`, pour ne pas charger supabase-js (environ 87 Ko compressés) sur les pages publiques. **supabase-js** (`src/lib/supabase.ts`) n'est chargé qu'à la demande, par `useAuth` (import dynamique), et seulement si une session est enregistrée (`sb-<projet>-auth-token` dans localStorage) ou à la connexion ; les modules admin (`useEventAdmin`, `useMilestoneAdmin`, `eventImages`) l'importent directement, ils sont dans des chunks admin. Ne pas importer `@/lib/supabase` depuis une vue ou un composable public. Les droits reposent entièrement sur RLS : lecture seule des lignes `published`. La saisie se fait dans le dashboard Supabase.
